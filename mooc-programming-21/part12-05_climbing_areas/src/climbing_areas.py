...
def sort_by_number_of_routes(items):
    def routes_a(item):
        return item.routes()
    return sorted(items,key=routes_a)

def sort_by_most_difficult(items):
    def routes_b(item):
        x= item.hardest_route()
        return x.grade
    return sorted(items,key=routes_b,reverse=True)
